...
export type StandardServiceRequestStatus =
  | 'pending'
  | 'resolved-valid'
  | 'resolved-invalid'
  | 'rejected'
...
export interface StandardServiceRequestEvidence {
  readonly id: number;
  readonly generation: number;
  readonly value: unknown;
  readonly status: StandardServiceRequestStatus;
}

interface DeferredRequest {
  readonly evidence: StandardServiceRequestEvidence;
  readonly resolve: (result: ValidationResult) => void;
  readonly reject: () => void;
  unsubscribeCancellation(): void;
}

export class StandardReferenceAsyncValidator {
  readonly validator: AsyncSchemaValidator;
  private requests: readonly StandardServiceRequestEvidence[] = Object.freeze(
    [],
  );
  ...
  constructor(
    ...
    private readonly notify: (
      evidence: readonly StandardServiceRequestEvidence[],
    ) => void,
  ) {
  ...
  getEvidence(): readonly StandardServiceRequestEvidence[] {
  ...
  private validate(
    ...
    const status: StandardServiceRequestStatus = this.throwNext
      ? 'threw'
      : 'pending';
    ...
    if (this.throwNext) {
      ...
      throw new Error('Deterministic Standard reference failure.');
    }
    ...
    const promise = new Promise<ValidationResult>((done, fail) => {
      resolve = done;
      reject = () => fail(new Error('Deterministic Standard rejection.'));
    });
  ...
  private replaceStatus(
    id: number,
    status: StandardServiceRequestStatus,
  ): void {
  ...
  private statusOf(id: number): StandardServiceRequestStatus | undefined {
